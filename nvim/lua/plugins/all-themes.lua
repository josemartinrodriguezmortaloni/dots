return {
  ...
  -- Preinstalled so switching to Token Meridian hot-reloads like the rest:
  -- lazy resolves the spec tree at startup, so a plugin declared only in the
  -- theme's own neovim.lua is not on disk during a live theme change.
  -- The name must match the one the theme spec uses.
  {
    "ThorstenRhau/token",
    name = "token",
    lazy = true,
